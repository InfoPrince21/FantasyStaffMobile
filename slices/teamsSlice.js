...
export const undoDraftedTeams = createAsyncThunk(
  ...
  async (id, { dispatch }) => {
    const records = await draftRecap.destroy(
      [id],
...
export const undoTeam1 = createAsyncThunk(
  "teams/undoTeam1",
  async (id, { dispatch }) => {
    const records = await team1draft.destroy(
      [id],
      function (err, deletedRecords) {
        if (err) {
          console.error(err);
...
export const undoTeam2 = createAsyncThunk(
  "teams/undoTeam2",
  async (id, { dispatch }) => {
    const records = await team2draft.destroy(
      [id],
      function (err, deletedRecords) {
        if (err) {
          console.error(err);
...
export const undoTeam3 = createAsyncThunk(
  "teams/undoTeam3",
  async (id, { dispatch }) => {
    const records = await team3draft.destroy(
      [id],
      function (err, deletedRecords) {
        if (err) {
          console.error(err);
...
const teamsSlice = createSlice({
  ...
  reducers: {
    removeTeam: (state, action) => {
      state.teamsArray = state.teamsArray.filter(
        (team) => team.id !== parseInt(action.payload)
      );
    },
    undoDraftTeam1: (state, action) => {
      state.draftTeam1Array = state.draftTeam1Array.filter(
        (team) => team.id !== action.payload
      );
      state.playerDrafted = false;
    },
    undoDraftTeam2: (state, action) => {
      state.draftTeam2Array = state.draftTeam2Array.filter(
        (team) => team.id !== action.payload
      );
    },
    undoDraftTeam3: (state, action) => {
      state.draftTeam3Array = state.draftTeam3Array.filter(
        (team) => team.id !== action.payload
      );
    },
    undoLastDraftedTeams: (state, action) => {
      state.dratedPlayers = state.dratedPlayers.filter(
        (team) => team !== action.payload
      );
    },
    setDraftTeam1: (state, action) => {
      const airTableRecords = action.payload;
      state.draftTeam1Array = airTableRecords.map((record) => ({
        id: record.id,
        fields: record.fields,
      }));
    },
    ...
    setAirTableTeams: (state, action) => {
      const airTableRecords = action.payload;
      state.teamsArray = airTableRecords.map((record) => ({
        id: record.id,
        fields: record.fields,
      }));
    },
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchDraftRecap.pending, (state) => {
        state.isLoading = true;
      })
      .addCase(fetchDraftRecap.fulfilled, (state, action) => {
        state.isLoading = false;
        state.errMsg = "";
        const airTableRecords = action.payload;
        state.draftedPlayers = airTableRecords.map((record) => ({
          id: record.id,
          fields: record.fields,
        }));
      })
      .addCase(fetchDraftRecap.rejected, (state, action) => {
        state.isLoading = false;
        state.errMsg = action.error ? action.error.message : "Fetch failed";
      })
      .addCase(fetchAirTableTeams.pending, (state) => {
        state.isLoading = true;
        state.draftedIds = [];
      })
      .addCase(fetchAirTableTeams.fulfilled, (state, action) => {
        state.isLoading = false;
        state.errMsg = "";
        const airTableRecords = action.payload;
        state.teamsArray = airTableRecords.map((record) => ({
      ...
      .addCase(fetchAirTableTeams.rejected, (state, action) => {
        state.isLoading = false;
        state.errMsg = action.error ? action.error.message : "Fetch failed";
      })
      .addCase(fetchTeam1Air.pending, (state) => {
        state.isLoading = true;
      })
      .addCase(fetchTeam1Air.fulfilled, (state, action) => {
        state.isLoading = false;
        state.errMsg = "";
        const airTableRecords = action.payload;
        state.draftTeam1Array = airTableRecords.map((record) => ({
      ...
      .addCase(fetchTeam1Air.rejected, (state, action) => {
        state.isLoading = false;
        state.errMsg = action.error ? action.error.message : "Fetch failed";
      })
      .addCase(fetchTeam2Air.pending, (state) => {
        state.isLoading = true;
      })
      .addCase(fetchTeam2Air.fulfilled, (state, action) => {
        state.isLoading = false;
        state.errMsg = "";
        const airTableRecords = action.payload;
        state.draftTeam2Array = airTableRecords.map((record) => ({
          id: record.id,
          fields: record.fields,
        }));
      })
      .addCase(fetchTeam2Air.rejected, (state, action) => {
        state.isLoading = false;
        state.errMsg = action.error ? action.error.message : "Fetch failed";
      })
      .addCase(fetchTeam3Air.pending, (state) => {
        state.isLoading = true;
      })
      .addCase(fetchTeam3Air.fulfilled, (state, action) => {
        state.isLoading = false;
        state.errMsg = "";
        const airTableRecords = action.payload;
        state.draftTeam3Array = airTableRecords.map((record) => ({
          id: record.id,
          fields: record.fields,
        }));
      })
      .addCase(fetchTeam3Air.rejected, (state, action) => {
        state.isLoading = false;
        state.errMsg = action.error ? action.error.message : "Fetch failed";
      })
      .addCase(draftTeam1AirTable.pending, (state) => {
        state.loadingDraft = true;
      })
      .addCase(draftTeam1AirTable.fulfilled, (state, action) => {
        state.loadingDraft = false;
        state.errMsg = "";
      })
      .addCase(draftTeam1AirTable.rejected, (state, action) => {
        state.loadingDraft = false;
        state.errMsg = action.error ? action.error.message : "Fetch failed";
      })
      .addCase(undoTeam1.pending, (state) => {
        state.loadingDraft = true;
      })
      .addCase(undoTeam1.fulfilled, (state, action) => {
        state.loadingDraft = false;
        state.errMsg = "";
      })
      .addCase(undoTeam1.rejected, (state, action) => {
        state.loadingDraft = false;
        state.errMsg = action.error ? action.error.message : "Fetch failed";
      });
    // Ensure to add all other async actions similarly
  },
});
...
export const selectFeaturedTeam = (state) => {
  return {
    featuredItem: state.teams.teamsArray.find(
      (team) => team.fields.featured === "true"
    ),
